...
class SessionTest {
    ...
    void 수강신청_모집중_상태() {
        ...
        Session session = Session.sessionWithState(0L, new Enrollment(students, null), recruitingPeriod);
        session.enrollStudent(NsUserTest.JAVAJIGI, null);
    ...
    void 수강신청_준비중_상태() {
        Session session = Session.sessionWithState(0L, new Enrollment(new Students(new ArrayList<>()), null), preparingPeriod);
    ...
    void 수강신청_종료_상태() {
        Session session = Session.sessionWithState(0L, new Enrollment(new Students(new ArrayList<>()), null), endPeriod);
    ...
    void 수강료와_결제금액_일치시_수강신청_완료() {
        Students students = new Students(new ArrayList<>());
        Session session = Session.recruitingPaidSession(0L, SessionType.PAID, new Enrollment(students, 5), 10000L);
        Payment payment = new Payment("ID", 0L, 0L, 10000L);

        session.enrollStudent(NsUserTest.JAVAJIGI, payment);

        assertThat(session.equals(Session.recruitingPaidSession(0L, SessionType.PAID, new Enrollment(students, 5),10000L)));
    }
}